      ...
      try {
        ...
        if (!user) {
          ...
          const result = await pool.query('INSERT INTO usuario SET ?', {
            ...
            email,
            tipo_Usuario: 'empleado', // Asigna un rol por defecto
          });

          // Recuperar el usuario recién insertado
          const [newUser] = await pool.query('SELECT * FROM usuario WHERE email = ?', [email]);
...
// Controlador de callback de Facebook
export const facebookCallback = (req: Request, res: Response) => {
  passport.authenticate('facebook', { session: false }, (err: Error | null, user: any) => {
    if (err || !user) {
      return res.redirect('/login');
    }
